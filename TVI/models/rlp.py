...
from .my_model import TextPromptderain
...
class RLP_NightRain(nn.Module):
    def __init__(self,model, in_c=3, out_c=3, use_rlp=False, use_rpim=False, dm_type='', rlp_feat=32, bias=False, opt=None):
        ...
        if use_rlp:
            ...
            self.recurrent_iter = 12 if self.dm_type == 'unet' else 4
            #self.rlp = PReNet_for_RLP(recurrent_iter=self.recurrent_iter, use_GPU=True)
            self.rlp = CLIP_encoder(model,recurrent_iter=self.recurrent_iter)
        ...
        # Deraining Module (DM)
        if self.dm_type == 'unet':
            self.dm = TextPromptderain(model,inp_channels=self.in_c, out_channels=self.out_c, dim =32,num_blocks = [4,6,6,8], num_refinement_blocks = 4,heads = [1,2,4,8],ffn_expansion_factor = 2.66,

...
class CsvDataset(Dataset):
    def __init__(self, input_filename, transforms, img_key, caption_key, sep="\t", tokenizer=None, da=False,
                 crop=False, prompt_learner=None):
        ...
        #self.prompt_learner = prompt_learner  # Initialize prompt learner
    ...
    def __getitem__(self, idx):
        ...
        # # Generate prompts using prompt learner if available
        # if self.prompt_learner is not None:
        #     prompts = self.prompt_learner()  # Assuming you want prompts for all classes
        #     texts = torch.cat([texts, prompts], dim=1)
...
def get_data(args, preprocess_fns, epoch=0, tokenizer=None):
    ...
    return data      #data 是一个字典，其中包含了至少一个键为 "train" 的数据集对象。